...
export const EditableNode = ({ data, selected }: CustomNodeProps) => {
  ...
  return (
    ...
    <div className={`relative p-3 rounded-md h-full w-full border-2 shadow-sm bg-[var(--obsidian-pane)] transition-all ${selected ? 'border-[var(--obsidian-accent)] shadow-md' : 'border-[var(--obsidian-border)]'}`}>
      <NodeResizer
        isVisible={selected}
        minWidth={120}
        minHeight={60}
